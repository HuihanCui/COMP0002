...
void drawMaze(int **maze, int size){
    setWindowSize(gridSize * size, gridSize * size);
    background();
    for (int x=0; x<size; x++){
        for (int y=0; y<size; y++){
            switch (maze[y][x]){
                case 0: setColour(gray); 
                    drawLine(x*gridSize, y*gridSize, (x+1)*gridSize,  y*gridSize);
                    drawLine(x*gridSize, y*gridSize, x*gridSize,  (y+1)*gridSize);
                    break;
                case 1: setColour(black); 
